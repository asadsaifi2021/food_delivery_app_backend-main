...
const checkUserExist = async (query) => {
  ...
  try {
    // Get the type of query (e.g., email or username)
    let queryType = Object.keys(query)[0];

    // Modify the query to be case-insensitive using regex
    let caseInsensitiveQuery = {
      [queryType]: { $regex: new RegExp(`^${query[queryType]}$`, "i") },
    };

    // Perform the case-insensitive search in MongoDB
    let userObject = await MongoDB.db
      .collection(mongoConfig.collections.USERS)
      .findOne(caseInsensitiveQuery);
